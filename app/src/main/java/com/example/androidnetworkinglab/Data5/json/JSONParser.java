package com.example.androidnetworkinglab.Data5.json;
import android.net.Uri;
import android.util.Log; import org.json.JSONObject;
import java.io.BufferedInputStream; import java.io.BufferedReader; import java.io.BufferedWriter; import java.io.InputStream;
...
public class JSONParser {
    ...
    public JSONObject makeHttpRequest(String duongdan, String method, List<HashMap<String, String>> params) {
        ...
        try {
            if (method.equals("POST")) {
            ...
            } else if (method.equals("GET")) { Uri.Builder builder = new Uri.Builder();
                int count = params.size();
                for (int i = 0; i < count; i++) {
                    for (Map.Entry<String, String> values : params.get(i).entrySet()) {
                        key = values.getKey();
                ...
                URL url = new URL(duongdan); HttpURLConnection conn = (HttpURLConnection)
                        url.openConnection();
        ...
        try {
            BufferedReader bufferedReader = new BufferedReader(new
                    InputStreamReader(is, StandardCharsets.UTF_8), 8);
            StringBuilder sb = new StringBuilder();
            String line = null;
            while ((line = bufferedReader.readLine()) != null) {
                sb.append(line + "\n"); }
            is.close();
            json = sb.toString();} catch (Exception ex) {
